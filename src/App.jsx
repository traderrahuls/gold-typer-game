...
/* ─── PROMO BANNER (rotates between referral messages every ~25s) ───
   Sab jagah jahan <AdBanner/> use hota hai (home, typing game, keyboard ke paas), yahi dikhega.
   Link change karna ho to PROMO_URL update kar do.
*/
const PROMO_URL = "https://bit.ly/m/traderrahul";
const PROMO_MESSAGES = [
  { icon:"🚀", title:"Get a Free Funded Trading Account", sub:"No investment — pass the challenge, trade our capital",
    bg:"linear-gradient(120deg,#f0b90b,#f59e0b)", color:"#1a1300", subColor:"#4a3900" },
  { icon:"📈", title:"Start Trading with Zero Commission", sub:"Open your account in 5 minutes, trade global markets",
    bg:"linear-gradient(120deg,#111827,#1f2937)", color:"#fff", subColor:"#9ca3af" },
];
...
const AdBanner = ({ slot="top" }) => {
  ...
  return (
    <a href={PROMO_URL} target="_blank" rel="noopener noreferrer" style={{
      textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"space-between",
      background:m.bg, borderRadius:compact?10:16, padding:compact?"9px 12px":"14px 16px",
      margin:"6px 0", boxShadow:"0 6px 16px rgba(0,0,0,.18)", flexShrink:0,
      minHeight:compact?36:60, transition:"background .5s ease",
    }}>
      <div style={{ display:"flex", alignItems:"center", gap:compact?7:10, overflow:"hidden" }}>
        <span style={{ fontSize:compact?16:24, flexShrink:0 }}>{m.icon}</span>
        <div style={{ overflow:"hidden" }}>
          <div style={{ fontWeight:800, fontSize:compact?11:13.5, color:m.color,
            whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.title}</div>
          {!compact && <div style={{ fontSize:10.5, color:m.subColor, marginTop:1 }}>{m.sub}</div>}
        </div>
      </div>
      <span style={{ fontSize:compact?14:20, color:m.color, flexShrink:0 }}>›</span>
    </a>
  );
};
...
const GAMES = [
  ...
  { id:"chess",      icon:"♞",  name:"Chess",              color:"#5c3a1e",     cat:"Strategy"  },
  { id:"fruitknife", icon:"🔪", name:"Fruit Knife Blast",  color:"#173a1c",     cat:"Arcade"    },
];
...
function Home({ onGame, scores }) {
  ...
  const streakDays = Number(localStorage?.getItem?.("gl_streak")) || 1;

  return (
    <div style={{ width:"100%", display:"flex", flexDirection:"column", background:"#fff", minHeight:"100%", ...px() }}>
      {/* header */}
      <div style={{ padding:"26px 18px 0", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          <div style={{ fontSize:11, color:"#9ca3af", fontWeight:700, letterSpacing:1 }}>{greeting}</div>
          <div style={{ fontSize:21, fontWeight:800, color:"#111827", marginTop:2 }}>Rahul</div>
        </div>
        <div style={{ background:"#111827", color:"#fff", fontSize:11, fontWeight:700,
          padding:"7px 12px", borderRadius:20 }}>🔥 {streakDays} day{streakDays!==1?"s":""}</div>
      </div>

      {/* typing text strip */}
      <div style={{ margin:"18px 18px 0", background:"#f4f5f7", borderRadius:12, padding:"10px 12px",
        display:"flex", gap:6, overflow:"hidden", whiteSpace:"nowrap" }}>
        <span style={{ fontSize:11, fontFamily:"monospace", color:C.blue, fontWeight:700 }}>gig</span>
        <span style={{ fontSize:11, fontFamily:"monospace", color:"#c7cad1" }}>life makes learning fun for everyone daily</span>
      </div>

      {/* hero card - featured game */}
      <div onClick={()=>onGame("typing","play")} style={{
        margin:"16px 18px 0", borderRadius:22, background:C.blue, position:"relative",
        overflow:"hidden", cursor:"pointer", minHeight:190, display:"flex", flexDirection:"column",
        justifyContent:"flex-end", boxShadow:`0 10px 26px ${C.blue}44`,
      }}>
        <div style={{ position:"absolute", top:10, right:12, fontSize:64, color:"rgba(255,255,255,.14)" }}>⌨</div>
        <div style={{ padding:"18px" }}>
          <div style={{ fontSize:10, color:"rgba(255,255,255,.7)", fontWeight:700, letterSpacing:1.3 }}>FEATURED GAME</div>
          <div style={{ fontSize:22, fontWeight:800, color:"#fff", marginTop:4 }}>Typing Blaster</div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,.8)", marginTop:3 }}>Sharpen your speed &amp; accuracy</div>
          <div style={{ marginTop:13, display:"flex", gap:10 }}>
            <div onClick={(e)=>{e.stopPropagation();onGame("typing","play");}} style={{ background:"#fff", color:C.blue,
              padding:"9px 20px", borderRadius:10, fontWeight:800, fontSize:12 }}>Play Now</div>
            <div onClick={(e)=>{e.stopPropagation();onGame("typing","loadtext");}} style={{ background:"rgba(255,255,255,.15)", color:"#fff",
              border:"1px solid rgba(255,255,255,.4)", padding:"9px 16px", borderRadius:10, fontWeight:700, fontSize:12 }}>📝 Load Text</div>
          </div>
        </div>
      </div>

      {/* Chess card */}
      <div onClick={()=>onGame("chess","play")} style={{
        margin:"14px 18px 0", borderRadius:22, background:"linear-gradient(135deg,#5c3a1e,#3c2610)",
        position:"relative", overflow:"hidden", cursor:"pointer", minHeight:170, display:"flex",
        flexDirection:"column", justifyContent:"flex-end", boxShadow:"0 10px 26px rgba(60,38,16,.45)",
        border:"1px solid rgba(240,185,11,0.25)",
      }}>
        <div style={{ position:"absolute", top:10, right:14, fontSize:60, color:"rgba(240,185,11,.5)" }}>♞</div>
        <div style={{ padding:"18px" }}>
          <div style={{ fontSize:10, color:"#f0b90b", fontWeight:700, letterSpacing:1.3 }}>STRATEGY GAME</div>
          <div style={{ fontSize:22, fontWeight:800, color:"#fdf6e3", marginTop:4 }}>Chess</div>
          <div style={{ fontSize:11, color:"#d9c9a3", marginTop:3 }}>Full rules, single &amp; multiplayer</div>
          <div onClick={(e)=>{e.stopPropagation();onGame("chess","play");}} style={{ marginTop:13, display:"inline-block", background:"#f0b90b", color:"#3c2610",
            padding:"9px 20px", borderRadius:10, fontWeight:800, fontSize:12 }}>Play Now</div>
        </div>
      </div>

      {/* Fruit Knife Blast card */}
      <div onClick={()=>onGame("fruitknife","play")} style={{
        margin:"14px 18px 0", borderRadius:22, background:"linear-gradient(135deg,#173a1c,#0c2410)",
        position:"relative", overflow:"hidden", cursor:"pointer", minHeight:170, display:"flex",
        flexDirection:"column", justifyContent:"flex-end", boxShadow:"0 10px 26px rgba(10,30,12,.45)",
        border:"1px solid rgba(255,181,71,0.22)",
      }}>
        <div style={{ position:"absolute", top:10, right:14, fontSize:56 }}>🔪</div>
        <div style={{ padding:"18px" }}>
          <div style={{ fontSize:10, color:"#ffb547", fontWeight:700, letterSpacing:1.3 }}>ARCADE GAME</div>
          <div style={{ fontSize:22, fontWeight:800, color:"#fff", marginTop:4 }}>Fruit Knife Blast</div>
          <div style={{ fontSize:11, color:"#bcd8bc", marginTop:3 }}>Slice fruits, dodge the bomb 💣</div>
          <div onClick={(e)=>{e.stopPropagation();onGame("fruitknife","play");}} style={{ marginTop:13, display:"inline-block", background:"#ffb547", color:"#3c2000",
            padding:"9px 20px", borderRadius:10, fontWeight:800, fontSize:12 }}>Play Now</div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, margin:"14px 18px 0" }}>
        <div style={{ flex:1, border:"1px solid #eef0f3", borderRadius:14, padding:"10px 12px" }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#111827" }}>{totalXP}</div>
          <div style={{ fontSize:8.5, color:"#9ca3af", fontWeight:700, letterSpacing:.5 }}>TOTAL XP</div>
        </div>
        <div style={{ flex:1, border:"1px solid #eef0f3", borderRadius:14, padding:"10px 12px" }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#111827" }}>Lv {level}</div>
          <div style={{ fontSize:8.5, color:"#9ca3af", fontWeight:700, letterSpacing:.5 }}>LEVEL</div>
        </div>
      </div>
...
/* ─── PROFILE ─── */
function Profile({ scores, darkMode, onToggleDark, user, profile={name:"Player",photo:""}, updateProfile=()=>{} }) {
  const totalXP = Object.values(scores).reduce((a,b)=>a+b,0);
  const level = Math.min(100, Math.floor(totalXP/100)+1);
  ...
  const [nameDraft, setNameDraft] = useState(profile.name || "Player");
  const fileRef = useRef(null);

  const onPickPhoto = (e) => {
    const file = e.target.files && e.target.files[0];
    if(!file) return;
    // compress via canvas so we don't blow up localStorage
    const img = new Image();
    ...
    reader.onload = (ev) => {
      img.onload = () => {
        ...
        updateProfile({ photo: canvas.toDataURL("image/jpeg",0.8) });
      };
    ...
    <div style={{ width:"100%", ...px() }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${C.blue},${C.purple})`, padding:"36px 16px 24px", textAlign:"center" }}>
        ...
        <div onClick={()=>fileRef.current&&fileRef.current.click()} style={{
          ...
          background: profile.photo ? `url(${profile.photo}) center/cover` : "#dbeafe",
          display:"flex", alignItems:"center", justifyContent:"center", position:"relative",
          fontSize:40, margin:"0 auto 10px", border:"3px solid rgba(255,255,255,.4)", cursor:"pointer" }}>
          {!profile.photo && "🧒"}
          <div style={{ position:"absolute", bottom:-2, right:-2, background:"#fff", borderRadius:"50%",
            width:22, height:22, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11,
            boxShadow:"0 2px 6px rgba(0,0,0,.25)" }}>📷</div>
        </div>
        {editingName ? (
          <input
            autoFocus
            ...
            onBlur={()=>{updateProfile({name:(nameDraft.trim()||"Player")});setEditingName(false);}}
            onKeyDown={e=>{if(e.key==="Enter")e.target.blur();}}
            style={{ color:"#fff", fontWeight:800, fontSize:18, background:"rgba(255,255,255,.15)",
              border:"1px solid rgba(255,255,255,.4)", borderRadius:8, textAlign:"center", padding:"3px 10px", outline:"none" }}
          />
        ) : (
          <div onClick={()=>{setNameDraft(profile.name||"Player");setEditingName(true);}} style={{ color:"#fff", fontWeight:800, fontSize:18, cursor:"pointer" }}>
            {profile.name||"Player"} <span style={{fontSize:12,opacity:.7}}>✏️</span>
          </div>
        )}
...
const WORD_BANK_MIXED = [...WORD_BANK_TRADING, ...WORD_BANK_ENGLISH];
...
function TypingGame({ onBack, addScore, startMode="play", profile={name:"Player",photo:""} }) {
  const [screen, setScreen]             = useState(startMode==="loadtext" ? "loadtext" : "playing"); // loadtext | playing | waveclear | gameover
  ...
  const launchGame = useCallback((mode, diff, customWaves) => {
    ...
    wordsDoneR.current=0; startTimeR.current=Date.now();
    setGameMode(mode); setDifficulty(diff);
    startMusic();
    startWave(waveList, 0, diff);
  },[getCtx,startMusic,startWave]);

  // Auto-launch straight into the game (no mode/difficulty select screen)
  useEffect(()=>{
    if(startMode!=="loadtext") launchGame("mixed","normal",null);
  ...
  const endGame = useCallback(()=>{
    ...
    const mins = Math.max(0.15, (Date.now()-startTimeR.current)/60000);
    setFinalWpm(Math.max(0, Math.round(wordsDoneR.current / mins)));
    setScreen("gameover");
  ...
  return (
    <div style={{position:"relative",width:"100%",minHeight:"100vh",fontFamily:"'Poppins','Segoe UI',sans-serif",overflow:"hidden",userSelect:"none",background:G.bg,display:"flex",flexDirection:"column",outline:wrongFlash?"2px solid "+G.red:"none",transform:shake?"translate(3px,-2px)":"none",transition:"transform 0.04s"}}>
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {/* Game Area */}
        <div ref={areaRef} style={{flex:1,position:"relative",overflow:"hidden",minHeight:280}}>
          ...
          {screen==="loadtext"&&(
            <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 16px",overflowY:"auto"}}>
              ...
              <div style={{display:"flex",gap:10,marginTop:16}}>
                ...
                <button onClick={onBack} style={{background:G.bgCard,border:"1px solid "+G.border,borderRadius:10,color:G.textMid,padding:"11px 20px",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Back</button>
              </div>
            </div>
          )}
          ...
          {/* GAME OVER */}
          {screen==="gameover"&&(
            <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:20,background:"rgba(10,16,32,0.94)",overflowY:"auto"}}>

              {/* ── Shareable score card ── */}
              <div id="share-card" style={{
                width:"100%", maxWidth:320, borderRadius:22, padding:"22px 20px 18px",
                background:"linear-gradient(155deg,#0b1120 0%,#111827 55%,#1e3a8a 100%)",
                border:"1px solid rgba(255,255,255,0.08)", boxShadow:"0 12px 30px rgba(0,0,0,0.4)",
                position:"relative", overflow:"hidden",
              }}>
                <div style={{position:"absolute",width:180,height:180,borderRadius:"50%",background:"#f0b90b",opacity:0.16,filter:"blur(50px)",top:-60,left:-60}}/>

                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative"}}>
                  <div style={{fontFamily:"inherit",fontSize:15,fontWeight:800,color:G.white}}>Gig<span style={{color:G.yellow}}>Life</span></div>
                  <div style={{fontSize:9,letterSpacing:2,color:G.red,fontWeight:700}}>GAME OVER</div>
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginTop:16,position:"relative"}}>
                  ...
                  <div style={{textAlign:"left"}}>
                    <div style={{fontSize:14,fontWeight:800,color:G.white}}>{profile.name||"Player"}</div>
                    <div style={{fontSize:9,color:G.textDim,letterSpacing:1}}>TYPING BLASTER RESULT</div>
                  </div>
                </div>

                <div style={{marginTop:20,position:"relative"}}>
                  <div style={{fontSize:56,fontWeight:900,color:G.yellow,lineHeight:1}}>{finalWpm}</div>
                  <div style={{fontSize:10,color:G.textDim,letterSpacing:3,marginTop:2}}>WORDS PER MINUTE</div>
                </div>

                <div style={{display:"flex",gap:8,marginTop:16,position:"relative"}}>
                  {[{label:"SCORE",val:scoreStr},{label:"BEST COMBO",val:"×"+bestCombo},{label:"WAVE",val:waveNum}].map(s=>(
                    <div key={s.label} style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 4px"}}>
                      <div style={{fontSize:13,color:G.white,fontWeight:800}}>{s.val}</div>
                      <div style={{fontSize:7.5,color:G.textDim,letterSpacing:.5,marginTop:2}}>{s.label}</div>
                    </div>
                  ))}
                </div>

                <div style={{marginTop:16,fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,letterSpacing:.5,position:"relative"}}>play.giglife.in</div>
              </div>

              <div style={{fontSize:10.5,color:G.textDim,marginTop:12,marginBottom:6}}>📸 Screenshot lekar apni story mein lagao!</div>

              <button onClick={()=>launchGame(gameMode,difficulty,null)} style={{background:"linear-gradient(135deg,"+G.blue+","+G.purple+")",color:G.white,border:"none",padding:"12px 44px",fontSize:14,fontWeight:700,letterSpacing:2,borderRadius:12,cursor:"pointer",fontFamily:"inherit",marginTop:8,marginBottom:12,boxShadow:"0 0 20px rgba(37,99,235,0.3)"}}>PLAY AGAIN</button>
        ...
        {screen==="playing"&&(
          <div style={{padding:"6px 16px",background:"rgba(10,16,32,0.9)",borderTop:"1px solid "+G.border,minHeight:34,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            ...
            {wordHint&&<span style={{fontSize:11,color:G.textDim}}>→ <span style={{color:"rgba(37,99,235,0.7)"}}>{wordHint}</span></span>}
          </div>
        )}

        {/* KEYBOARD */}
        {screen==="playing"&&(
          <div style={{padding:"10px 8px 16px",background:"rgba(10,16,32,0.95)",borderTop:"1px solid "+G.border,flexShrink:0}}>
            {KEY_ROWS.map((row,ri)=>(
              <div key={ri} style={{display:"flex",justifyContent:"center",gap:6,marginBottom:7,paddingLeft:ri===1?18:ri===2?36:0,paddingRight:ri===1?18:ri===2?36:0}}>
                {row.map(k=>{
                  const live=liveLetters.has(k),ind=!live&&indLetters.has(k),act=activeKey===k;
                  return(
                    <button key={k} onPointerDown={e=>{e.preventDefault();pressKey(k);}} style={{flex:1,maxWidth:46,minWidth:24,height:58,borderRadius:9,fontWeight:700,fontSize:16,textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",touchAction:"manipulation",transition:"all 0.06s",border:"1px solid "+(act||live?G.blue:ind?"rgba(37,99,235,0.3)":"rgba(255,255,255,0.08)"),background:act?G.blue:live?"rgba(37,99,235,0.2)":ind?"rgba(37,99,235,0.07)":"rgba(255,255,255,0.04)",color:act?G.white:live?G.blue:ind?"rgba(37,99,235,0.6)":"rgba(255,255,255,0.25)",boxShadow:act?"0 0 12px rgba(37,99,235,0.5)":live?"0 0 6px rgba(37,99,235,0.2)":"none"}}>{k}</button>
                  );
                })}
              </div>
            ))}
...
export default function App() {
  ...
  const [startMode,setStartMode]=useState("play");
  const [profile,setProfile]=useState(()=>{
    try{
      const saved=localStorage.getItem("gl_profile");
      return saved ? JSON.parse(saved) : { name:"Player", photo:"" };
    }catch(e){ return { name:"Player", photo:"" }; }
  });
  const updateProfile=(p)=>{
    setProfile(prev=>{
      const next={...prev,...p};
      try{ localStorage.setItem("gl_profile", JSON.stringify(next)); }catch(e){}
      return next;
    });
  };
  const [scores,setScores]=useState({typing:0, chess:0, fruitknife:0});
  ...
  const GAME_COMPS={typing:TypingGame, chess:ChessGame, fruitknife:FruitKnifeBlast};
    ...
    <div style={{ maxWidth:430,margin:"0 auto",minHeight:"100vh",boxShadow:"0 0 40px rgba(0,0,0,.15)",overflow:"hidden",background:"#fff",
      ...
      {phase==="app" && (
        <div style={{ display:"flex",flexDirection:"column",minHeight:"100vh" }}>
          {game ? (
            (() => { const G=GAME_COMPS[game]; return <G onBack={()=>setGame(null)} addScore={addScore} startMode={startMode} profile={profile}/>; })()
          ) : (
            <div style={{ flex:1,overflowY:"auto" }}>
              {tab==="home"        && <Home onGame={(id,mode="play")=>{setStartMode(mode);setGame(id);}} scores={scores}/>}
              {tab==="games"       && <GamesPage onGame={(id)=>{setStartMode("play");setGame(id);}}/>}
              {tab==="leaderboard" && <Leaderboard/>}
              {tab==="profile"     && <Profile scores={scores} darkMode={darkMode} onToggleDark={()=>setDarkMode(d=>!d)} user={user} profile={profile} updateProfile={updateProfile}/>}
            </div>
          )}
