...
/* ─── GOOGLE ADSENSE ───
   1) Apna Publisher ID yahan daalo (AdSense approve hone ke baad milta hai)
   2) Har jagah jahan <AdBanner/> use hota hai, uska apna alag "slotId" AdSense dashboard se banao aur pass karo,
      e.g. <AdBanner slotId="1234567890" />
*/
const ADSENSE_CLIENT = "ca-pub-XXXXXXXXXXXXXXXX"; // 👈 apna Publisher ID yahan daalo

const AdBanner = ({ slot="top", slotId }) => {
  const ref = useRef(null);
  useEffect(()=>{
    if(!slotId) return; // slotId na ho to kuch render nahi hota (avoid blank/broken ad)
    try{
  ...
  if(!slotId){
    // Dev/placeholder view — dikhta hai jab tak real slotId nahi diya gaya
    return (
      <div style={{
      ...
      }}>📢 AD SLOT ID MISSING ({slot})</div>
    );
  }

  return (
    <div style={{ width:"100%", margin:"6px 0", flexShrink:0, minHeight: slot==="top"?50:90, overflow:"hidden" }}>
  ...
  return (
    <div style={{ width:"100%", display:"flex", flexDirection:"column", background:"#fff", minHeight:"100%", ...px() }}>
      ...
      <div style={{ margin:"14px 18px 0", display:"flex", flexDirection:"column", gap:10 }}>
        <a href="https://bit.ly/m/traderrahul" target="_blank" rel="noopener noreferrer" style={{
          textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"space-between",
          background:"linear-gradient(120deg,#f0b90b,#f59e0b)", borderRadius:16, padding:"14px 16px",
          boxShadow:"0 8px 20px rgba(240,185,11,.35)",
        }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:24 }}>🚀</span>
            <div>
              <div style={{ fontWeight:800, fontSize:13.5, color:"#1a1300" }}>Get a Free Funded Trading Account</div>
              <div style={{ fontSize:10.5, color:"#4a3900", marginTop:1 }}>No investment — pass the challenge, trade our capital</div>
            </div>
          </div>
          <span style={{ fontSize:20, color:"#1a1300" }}>›</span>
        </a>

        <a href="https://bit.ly/m/traderrahul" target="_blank" rel="noopener noreferrer" style={{
          textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"space-between",
          background:"linear-gradient(120deg,#111827,#1f2937)", borderRadius:16, padding:"14px 16px",
          boxShadow:"0 8px 20px rgba(0,0,0,.25)",
        }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:24 }}>📈</span>
            <div>
              <div style={{ fontWeight:800, fontSize:13.5, color:"#fff" }}>Start Trading with Zero Commission</div>
              <div style={{ fontSize:10.5, color:"#9ca3af", marginTop:1 }}>Open your account in 5 minutes, trade global markets</div>
            </div>
          </div>
